...
export const removeUndefinedAndNull = <T>(
  array: Array<null | undefined | T>
): T[] => {
  return (
    array.filter(
      (item): item is T => item !== null && typeof item !== 'undefined'
    ) || []
  );
};
export const uniqueArray = (things: object[]) =>
...
export const singleCountry = (countries: object[], searchTerm: string) =>
  countries.filter((item: any) => item.code === searchTerm);
...
const replacerSetToJSON = (_: string, value: any) =>
  typeof value === 'object' && value instanceof Set ? Array.from(value) : value;

export const printJson = (obj: object) =>
  JSON.stringify(obj, replacerSetToJSON, 2);
